...
<select id="mode">
  <option value="backup">备份模式（只上传，不修改本机配置）</option>
  <option value="sync">同步模式（双向同步，会写回本机配置）</option>
</select>
...
document.getElementById('save').onclick=()=>{const configuration={};for(const id of ids)configuration[id]=document.getElementById(id).value;const automation={mode:document.getElementById('mode').value,autoSync:document.getElementById('autoSync').checked,includeProfileAssociations:document.getElementById('includeProfileAssociations').checked,debounceSeconds:Number(document.getElementById('debounceSeconds').value),pollIntervalSeconds:Number(document.getElementById('pollIntervalSeconds').value)};vscode.postMessage({type:'save',configuration,automation})};
document.getElementById('sync').onclick=()=>vscode.postMessage({type:'sync'});
addEventListener('message',({data})=>{if(data.type!=='state')return;for(const id of ids)document.getElementById(id).value=data.configuration[id]??'';document.getElementById('mode').value=data.configuration.mode;document.getElementById('autoSync').checked=data.configuration.autoSync;document.getElementById('includeProfileAssociations').checked=data.configuration.includeProfileAssociations;document.getElementById('debounceSeconds').value=String(data.configuration.debounceSeconds);document.getElementById('pollIntervalSeconds').value=String(data.configuration.pollIntervalSeconds);const s=data.status;document.getElementById('phase').textContent=s.displayPhase+' · '+s.role;document.getElementById('detail').textContent='窗口 '+s.activeWindows+' · Profiles '+s.profiles.join('、')+(s.message?' · '+s.message:'');document.getElementById('syncPhase').textContent=s.displayPhase;document.getElementById('syncMode').textContent=modeLabel(data.configuration.mode);document.getElementById('modeNote').textContent=modeNote(data.configuration.mode);const dot=document.getElementById('syncDot');dot.className='dot '+syncClass(s.displayPhase);lastSyncAt=s.lastSyncAt;renderLastSyncAt()});
...
function modeLabel(mode){return mode==='sync'?'同步模式':'备份模式'}
function modeNote(mode){return mode==='sync'?'同步模式会把仓库内容写回本机；如果同时开启 IDE 内置 Settings Sync，可能出现互相覆盖。':'备份模式只把本机配置提交到仓库，不会写回本机，建议在开启 IDE 内置 Settings Sync 时使用。'}
</script></body></html>`;
  }
}
