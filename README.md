...
每轮同步先保存拉取前的仓库快照作为基础版本，再拉取云端最新版本，并将基础版本、本机快照和云端快照逐个文件比较：
...
窗口不安全、独占锁被其他窗口占用等可恢复情况会延迟重试；配置冲突会等待用户处理；其他失败会显示具体原因。本轮未完整成功时不会更新“上次成功同步”时间。

### 插件自身配置

仓库地址、分支、Git 提交身份和自动同步参数使用独立的版本化记录，不参与上述 Profile 文件冲突流程。只有一方更新时接受因果关系中较新的版本；多设备并发修改且内容不同时，以当前机器共享的插件配置为准，合并双方版本历史后写回宿主 Settings Sync。旧版本遗留的配置冲突和恢复记录会在启动时静默清理，不再显示恢复备份黄框。
