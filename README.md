...
- 同步开关默认关闭，需要在侧边栏显式开启：本机没有仓库时首轮同步会整包采用云端配置覆盖本机，不应在用户无感知时发生。关闭后不创建本地仓库、不访问远程仓库，也不写回本机配置。
- 开关、上次同步时间和本机与远端的关系存放在扩展运行目录的共享文件中，同一设备的全部窗口与 Profiles 共用一份，但不会同步到其他设备。
- 工作区/Profile 关联关系默认开启；侧边栏可设置本地检测与远程轮询间隔，默认分别为 10 秒和 300 秒。

## 同步策略

### 触发与安全检查

同步开关开启后，IDE 启动、本地 Profile 指纹变化、远程轮询和命令面板中的“立即同步”都会触发同步；开关关闭时这些触发一律不执行，“立即同步”会提示先开启同步。同一 User Data 目录中只有 leader 窗口执行 Git、AI 和 Profile 写入操作，follower 窗口只把请求转发给 leader。另一窗口正在执行同步时，当前请求会等待后续重试。侧边栏保存配置后自动同步；本地缓存与远端不同源时，leader 会询问是否废弃缓存并按云端覆盖本机。
...
仓库地址、分支、Git 提交身份和同步间隔参数使用独立的版本化记录，不参与上述 Profile 文件冲突流程。只有一方更新时接受因果关系中较新的版本；多设备并发修改且内容不同时，以当前机器共享的插件配置为准，合并双方版本历史后写回宿主 Settings Sync。旧版本遗留的配置冲突和恢复记录会在启动时静默清理，不再显示恢复备份黄框。`profileGitSync.*` 设置项不会随 `settings.json` 提交到仓库，避免与版本化记录互相覆盖。
