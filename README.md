...
- 配置同步仓库地址、分支、Git 提交身份、运行模式和自动同步参数在同一设备的全部窗口与 Profiles 间保持一致；启用宿主 Settings Sync 后，也会在同类 IDE 的设备间自动恢复。
...
- 同步过程不弹通知，状态只出现在状态栏图标和侧边栏概览中；侧边栏保留当前状态和上次成功同步时间，时间在 IDE 重启后保留。
- 侧边栏可直接开关自动同步并设置本地检测与远程轮询间隔；默认分别为 10 秒和 300 秒。
...
首次同步没有基础版本时按空快照处理。只存在于一侧的文件会自动进入合并结果；双方同一路径存在不同内容时视为冲突。Profile 列表、元数据以及可选的工作区关联关系遵循相同规则。Profile 增删只会在仅有一个活动窗口时写入本机并自动重载窗口，多窗口时保持待应用状态。
...
插件只暂存和提交 `.profile-git-sync/hosts/<宿主>`，不会操作当前工作区项目的 Git 仓库，也不会 force push。同步模式下，合并结果推送成功后才恢复本机 Profile 文件；涉及 `extensions.json` 时会等待 IDE 安装扩展，未装完也不会阻塞或提示重载。Profile 结构变化在单窗口时自动应用，应用后静默重载窗口。备份模式下不会写回本机，因此不会触发扩展装卸和窗口重载。
