...
插件只暂存和提交 `.profile-git-sync/hosts/<宿主>`，不会操作当前工作区项目的 Git 仓库，也不会 force push。合并结果推送成功后才恢复本机 Profile 文件；涉及 `extensions.json` 时会等待 IDE 安装扩展，未装完也不会阻塞或提示重载。Profile 结构变化在单窗口时自动应用，应用后静默重载窗口。
