...
同步开关开启后，IDE 启动、本地 Profile 指纹变化和命令面板中的“立即同步”都会触发同步；同步模式下还会按间隔轮询远程仓库。开关关闭时这些触发一律不执行，“立即同步”会提示先开启同步。同一 User Data 目录中只有 leader 窗口执行 Git、AI 和 Profile 写入操作，follower 窗口只把请求转发给 leader。另一窗口正在执行同步时，当前请求会等待后续重试。侧边栏保存配置后自动同步；本地缓存与远端不同源时，leader 会询问是否废弃缓存：同步模式按云端覆盖本机，备份模式则重新克隆后继续把本机配置备份到云端。
...
| 云端等于基础版本 | 采用本机修改或删除 |
| 本机与云端结果相同 | 直接采用该结果 |
| 本机与云端都相对基础版本发生不同修改 | 优先 AI 合并，AI 不可用或结果无效时保留本机版本（本机已删除时采用云端），并备份两份原始快照 |

首次同步没有基础版本时按空快照处理。只存在于一侧的文件会自动进入合并结果；双方同一路径存在不同内容时视为冲突。Profile 列表、元数据以及工作区关联关系遵循相同规则。Profile 增删只会在仅有一个活动窗口时写入本机并自动重载窗口，多窗口时保持待应用状态。

### 冲突处理

冲突不会中断同步，也不需要用户选择：

- **AI 合并**：AI 基于基础、本机和云端内容生成候选，候选必须通过路径、JSON/JSONC 格式和凭据检查后才会采用。冲突内容疑似包含凭据时不会发送给模型。
- **确定性回退**：宿主没有开放模型、AI 超时或候选未通过检查时，改为保留本机版本；本机已删除该文件时采用云端版本。Profile 结构冲突同样回退到本机，避免删除本机正在使用的 Profile。
- **备份**：只要本轮出现冲突，合并前的本机与云端完整快照都会存入扩展运行目录的 `conflict-backups`，只保留最近 5 份。

同一轮同步中 AI 只尝试一次，一旦失败后续冲突直接走确定性回退，避免反复等待模型超时。

### 提交、应用与失败处理

插件只暂存和提交 `.profile-git-sync/hosts/<宿主>`，不会操作当前工作区项目的 Git 仓库，也不会 force push。同步模式下，合并结果推送成功后才恢复本机 Profile 文件；涉及宿主级 `extensions.json` 时会调用 IDE 安装缺失扩展、卸载云端已移除的扩展并等待生效，未装完也不会阻塞或提示重载。Profile 结构变化在单窗口时自动应用，应用后静默重载窗口。备份模式不写回本机，因此不会触发扩展装卸和窗口重载。

窗口不安全、独占锁被其他窗口占用等可恢复情况会延迟重试，重试间隔逐步退避；失败原因显示在状态栏与侧边栏，不弹出通知。本轮未完整成功时不会更新“上次成功同步”时间。

推送失败会在本地留下未推送的提交。下一轮同步时若这些提交无法自动变基到云端最新版本，插件会丢弃它们并重新对齐云端；同步模式下再用磁盘上的本机配置走一次三方合并，因此不会停留在无法恢复的失败状态。恢复本机文件时产生的备份保存在扩展运行目录，只保留最近 10 份。
